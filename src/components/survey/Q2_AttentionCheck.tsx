...
import { Q2_AttentionChecks as config } from '@/config/surveyQuestions';
...
export default function Q2_AttentionCheck({ product, onComplete }: Q2Props) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const allAnswered = config.items.every(item => 
      responses[item.variable as keyof AttentionCheckResponse] !== undefined
    );

    if (allAnswered) {
      onComplete(responses as AttentionCheckResponse);
    }
  };
            ...
            <div key={item.variable} className="space-y-3">
              ...
              {item.warning && (
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
                  <p className="text-yellow-800 text-sm font-medium">{item.warning}</p>
                </div>
              )}
